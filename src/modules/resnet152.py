...
class Bottleneck(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.conv2 = torch.nn.Conv1d(out_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = torch.nn.BatchNorm1d(out_channels)
        self.conv3 = torch.nn.Conv1d(out_channels, self.expansion*out_channels, kernel_size=1, bias=False)
        self.bn3 = torch.nn.BatchNorm1d(self.expansion*out_channels)

        self.shortcut = torch.nn.Sequential()
        if stride != 1 or in_channels != self.expansion*out_channels:
            self.shortcut = torch.nn.Sequential(
                torch.nn.Conv1d(in_channels, self.expansion*out_channels, kernel_size=1, stride=stride, bias=False),
...
class ResNet(torch.nn.Module):
    def __init__(self, block, num_blocks, num_classes):
        ...
        self.conv1 = torch.nn.Conv1d(1000, 64, kernel_size=7, stride=2, padding=3, bias=False)
